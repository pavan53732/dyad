...
const SandboxEnforcementArgs = z.object({
  ...
  limits: z
    .object({
      ...
      tokens: z.number().optional().describe("Token quota for the session"),
    })
    .optional(),
});
...
interface GovernanceData {
  ...
  quotas?: Record<string, TokenUsageBucket>;
}
...
function loadGovernance(ctx: AgentContext): GovernanceData {
  ...
  if (!fs.existsSync(filePath)) {
    return { permissions: [], auditLogs: [], activePolicies: [], quotas: {} };
  }
  try {
...
export const agentPermissionSystemTool: ToolDefinition<AgentPermissionArgs> = {
  ...
  execute: async (args, ctx) => {
    ...
    if (action === "set") {
      if (!capability || !level)
        throw new Error("Capability and Level are required for set action.");
      const index = data.permissions.findIndex(
        (p) => p.target === target && p.capability === capability,
      );
      if (index >= 0) {
    ...
    if (action === "revoke") {
      if (!capability)
        throw new Error("Capability is required for revoke action.");
...
export const agentActionAuditTool: ToolDefinition<AgentAuditArgs> = {
  ...
  description:
    "Log and query agent actions for safety and compliance auditing.",
  ...
  execute: async (args, ctx) => {
    ...
    if (action === "report") {
      const summary = {
        ...
        critical: data.auditLogs.filter((l) => l.severity === "critical")
          .length,
      };
    ...
    if (filter)
      filtered = filtered.filter(
        (l) => l.agentId.includes(filter) || l.action.includes(filter),
      );
...
export const agentSandboxEnforcementTool: ToolDefinition<SandboxEnforcementArgs> =
  {
    ...
    description:
      "Enforce safety boundaries and resource restrictions for AI agents.",
    ...
    execute: async (args, ctx) => {
      ...
      if (action === "verify") {
        ...
        return isWithinRoot
          ? "Target is within sandbox boundaries."
          : "VIOLATION: Target escapes sandbox.";
      }
      ...
      if (action === "restrict") {
        ...
        if (limits) {
          ...
          data.quotas[target] = {
            tokensUsed: 0,
            memoryUsed: limits.memory || 0,
            lastUpdate: new Date().toISOString(),
          };
          saveGovernance(ctx, data);
        }
        return `Restrictions applied to ${target}: CPU=${limits?.cpu || "N/A"}, MEM=${limits?.memory || "N/A"}MB, TOKENS=${limits?.tokens || "N/A"}`;
      }

      if (action === "monitor") {
        const data = loadGovernance(ctx);
        const quota = data.quotas?.[target];
        if (quota) {
          if (quota.memoryUsed > (limits?.memory || 1024)) {
            return `QUOTA VIOLATION: Agent ${target} exceeded memory limit. Access throttled.`;
          }
          return `Monitoring ${target}: Status=Healthy, Memory=${quota.memoryUsed}MB`;
        }
      }
