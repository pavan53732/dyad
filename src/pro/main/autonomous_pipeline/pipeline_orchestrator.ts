...
export class PipelineOrchestrator {
    ...
    this.scheduler = new AgentScheduler(
      ...
      },
      {
        ...
        onEntryCompleted: (entry, result) =>
          this.handleTaskCompleted(entry, result),
  ...
  async execute(
    ...
    logger.info(
      `Starting autonomous pipeline for request: ${request.substring(0, 100)}...`,
    );
    ...
    try {
      ...
      if (this.config.enableProactiveKnowledge) {
        ...
        this.state.knowledgeContext = await this.buildProactiveKnowledgeContext(
          request,
          context.appId,
        );
      ...
      // PHASE 2: Planning
      if (
        this.config.enableAutoPlanning &&
        this.shouldGeneratePlan(request, this.state.knowledgeContext)
      ) {
        this.state.phase = "planning";

        // Enhance context with knowledge
        const enhancedContext = this.enhanceContextWithKnowledge(
          context,
          this.state.knowledgeContext,
        );

        // Generate plan
        this.state.plan = await this.planningEngine.generatePlan(
          request,
          enhancedContext,
        );
      ...
      this.emitEvent({
        ...
        data: {
          duration:
            this.state.completedAt.getTime() -
            (this.state.startedAt?.getTime() || 0),
          taskCount: this.state.executionResults.size,
          successCount: Array.from(this.state.executionResults.values()).filter(
  ...
  private async buildProactiveKnowledgeContext(
    ...
    return {
      taskIntent,
      relevantEntities: entities.slice(
        0,
        this.config.maxKnowledgeContextEntities,
      ),
  ...
  private extractTaskIntent(request: string): TaskIntent {
    ...
    if (lower.includes("complex") || lower.includes("comprehensive"))
      complexity = 7;
    if (lower.includes("architecture") || lower.includes("redesign"))
      complexity = 8;
      ...
      if (entity.relationships) {
        for (const rel of entity.relationships) {
          insights.push({
            ...
            type: rel.type,
            impact:
              rel.type === "imports" || rel.type === "calls" ? "medium" : "low",
          });
        }
      }
    }
  ...
  private shouldGeneratePlan(
    ...
    if (
      knowledgeContext?.taskIntent.complexity &&
      knowledgeContext.taskIntent.complexity >=
        this.config.planningComplexityThreshold
    ) {
      return true;
    }
  ...
  private async schedulePlan(
    ...
    logger.info(
      `Scheduled ${plan.tasks.length} tasks for plan ${plan.plan.id}`,
    );
  }
  ...
  private async waitForCompletion(): Promise<void> {
    return new Promise((resolve) => {
      const checkInterval = setInterval(() => {
        const pending = Array.from(this.state.executionResults.values()).filter(
  ...
  private async recordLearningOutcomes(appId: number): Promise<void> {
    ...
    const failedTasks = Array.from(this.state.executionResults.values()).filter(
      (r) => r.status === "failed",
    );
  ...
  private extractLessons(): string[] {
    ...
    const avgDuration =
      Array.from(this.state.executionResults.values())
        .filter((r) => r.duration !== undefined)
        .reduce((sum, r) => sum + (r.duration || 0), 0) /
      this.state.executionResults.size;
  ...
  getKnowledgeContextInjection(): string {
    ...
    // Task intent
    parts.push(
      `[KNOWLEDGE CONTEXT - Task Intent: ${ctx.taskIntent.type} (confidence: ${ctx.taskIntent.confidence.toFixed(2)})]`,
    );
    ...
    if (ctx.relevantEntities.length > 0) {
      ...
      for (const entity of ctx.relevantEntities.slice(0, 10)) {
        parts.push(
          `- ${entity.name} (${entity.type}) in ${entity.filePath || "unknown"}`,
        );
    ...
    if (ctx.relatedDecisions.length > 0) {
      ...
      for (const decision of ctx.relatedDecisions.slice(0, 5)) {
        parts.push(
          `- ${decision.title}: chose "${decision.selectedOption}" (${decision.type})`,
        );
      }
    }
    ...
    if (ctx.recommendations.length > 0) {
      ...
      for (const rec of ctx.recommendations) {
        parts.push(
          `- ${rec.suggestion} (confidence: ${rec.confidence.toFixed(2)})`,
        );
      }
    }
