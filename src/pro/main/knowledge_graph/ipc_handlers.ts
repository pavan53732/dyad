...
import type { KnowledgeNodeType, KnowledgeEdgeType, CodeGraphBuildOptions } from "./types";
...
export function registerKnowledgeGraphHandlers(): void {
  ...
  ipcMain.handle(KNOWLEDGE_GRAPH_CHANNELS.GET_NODE, async (_, nodeId: string) => {
    return graphStorage.getNode(nodeId);
  });

  ipcMain.handle(KNOWLEDGE_GRAPH_CHANNELS.QUERY_NODES, async (_, 
    appId: number, 
    filter?: { type?: KnowledgeNodeType | KnowledgeNodeType[]; filePath?: string | string[]; name?: string | string[] },
    options?: { limit?: number; offset?: number }
  ) => {
    return graphStorage.queryNodes(appId, filter, options);
  });

  ipcMain.handle(KNOWLEDGE_GRAPH_CHANNELS.DELETE_NODE, async (_, nodeId: string) => {
  ...
  ipcMain.handle(KNOWLEDGE_GRAPH_CHANNELS.GET_OUTGOING_EDGES, async (_, nodeId: string, types?: KnowledgeEdgeType[]) => {
    return graphStorage.getOutgoingEdges(nodeId, types);
  });

  ipcMain.handle(KNOWLEDGE_GRAPH_CHANNELS.GET_INCOMING_EDGES, async (_, nodeId: string, types?: KnowledgeEdgeType[]) => {
  ...
  ipcMain.handle(KNOWLEDGE_GRAPH_CHANNELS.BUILD_GRAPH, async (_, options: CodeGraphBuildOptions) => {
    return codeGraphBuilder.buildGraph(options);
  });

  ipcMain.handle(KNOWLEDGE_GRAPH_CHANNELS.UPDATE_GRAPH, async (_, options: CodeGraphBuildOptions) => {
    // For updates, we rebuild the entire graph for simplicity
    // In production, this would be incremental
    return codeGraphBuilder.buildGraph(options);
  });

  ipcMain.handle(KNOWLEDGE_GRAPH_CHANNELS.CLEAR_GRAPH, async (_, appId: number) => {
  ...
  ipcMain.handle(KNOWLEDGE_GRAPH_CHANNELS.FIND_NODES_BY_NAME, async (_, 
    appId: number, 
    pattern: string, 
  ...
  ipcMain.handle(KNOWLEDGE_GRAPH_CHANNELS.FIND_NODES_IN_FILE, async (_, 
    appId: number, 
    filePath: string,
  ...
  ipcMain.handle(KNOWLEDGE_GRAPH_CHANNELS.FIND_CALLERS, async (_, 
    nodeId: string,
  ...
  ipcMain.handle(KNOWLEDGE_GRAPH_CHANNELS.FIND_CALLEES, async (_, 
    nodeId: string,
    options?: { recursive?: boolean; maxDepth?: number }
  ) => {
  ...
  ipcMain.handle(KNOWLEDGE_GRAPH_CHANNELS.FIND_PATH, async (_, 
    startNodeId: string,
    endNodeId: string,
  ...
  ipcMain.handle(KNOWLEDGE_GRAPH_CHANNELS.EXTRACT_SUBGRAPH, async (_, options: {
  ...
  ipcMain.handle(KNOWLEDGE_GRAPH_CHANNELS.GET_STATS, async (_, appId: number) => {
    return graphStorage.getStats(appId);
  });

  ipcMain.handle(KNOWLEDGE_GRAPH_CHANNELS.ANALYZE_GRAPH, async (_, appId: number) => {
  ...
  ipcMain.handle(KNOWLEDGE_GRAPH_CHANNELS.CREATE_SNAPSHOT, async (_, 
    appId: number,
  ...
  ipcMain.handle(KNOWLEDGE_GRAPH_CHANNELS.GET_LATEST_SNAPSHOT, async (_, appId: number) => {
